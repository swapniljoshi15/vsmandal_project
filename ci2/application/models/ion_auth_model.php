...
class Ion_auth_model extends CI_Model
{
	...
	//**************code start*****************//
	function validate_function_access($function_name, $user_id ){
		//get function_id
		...
		if($query->num_rows() == 1){
			...
			$function_id = $function[0]->function_id;
			//now get group id for user
			...
			if($q->num_rows() > 0){
				...
				foreach($q->result() as $data){
					...
					$where = $where . 'OR';
					$count = true;
					$where = $where."( function_id=".$function_id." AND group_id=".$data->group_id.") ";
				}
				$Db->where($where);
				$q1 = $Db->get('function_access');
				if($q1->num_rows() > 0){
					return true;
				}
				else{
	...
	public function remove_from_function_access($group_id)
	{
		// user id is required
		if(empty($group_id))
		{
		...
		return $this->db->delete('function_access');

	}

	public function add_to_function_access($group_id, $function_ids)
	{
		// user id is required
		if(empty($group_id) && empty($function_id))
		{
			return FALSE;
		}
		$data = array();
		...
		$this->db->insert_batch('function_access', $data);
	...
	public function listAccessFunctionIds($id = NULL)
	{
		$data = array();
		if (isset($id))
		{
			$this->db->where('group_id', $id);
			$query = $this->db->get('function_access');
			if($query->num_rows()>0){
				$data = $query->result();
			}
		}
